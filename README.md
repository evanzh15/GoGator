...
Your designing documents should have a general answer for each of the following questions. 
...
2. What is the victory condition for the game? What is the player trying to achieve? 
	...
	Avoid obstacles and stay alive, whilst trying to collect as many coins (or coin equivalent items) and items as possible. There is no victory condition, just try to beat the high score (stored via file I/O).

3. What is the player's interaction model (mouse/keyboard)? 

	Mouse, specifically arrow keys will move the player model around the map. 
